...
onMounted(() => {
  retireProductStore.getRetireProductToApi()
})
</script>

<template>
  <div v-if="retireProductStore.isRetireProduct">
    <h2>Ils vous intéresse</h2>

    <Carousel
      :value="retireProductStore.retireProduct"
      :numVisible="3"
      :numScroll="1"
      :responsiveOptions="retireProductStore.responsiveOptions"
    >
      <template #item="slotProps">
        <routerLink
          :to="{ name: 'detailsProduct', params: { productId: slotProps.data.productID } }"
        >
